...
export default function App() {
  ...
  const syncUserProfile = async (tokenStr: string, firebaseUser: any) => {
    try {
      // 1. Sync user profile on backend PostgreSQL
      ...
      // 2. Fetch user's orders from Cloud SQL PostgreSQL
      ...
      // 3. Fetch user's wishlist from Cloud SQL PostgreSQL
      ...
      if (wishlistRes.ok) {
        const dbWishlistIds = await wishlistRes.json(); // Array of product ids
    ...
    if (isPresent) {
      ...
      if (idToken) {
        try {
          ...
          console.error('Failed to sync wishlist deletion to Cloud SQL:', err);
        }
      }
    } else {
      ...
      if (idToken) {
        try {
          ...
          console.error('Failed to sync wishlist insertion to Cloud SQL:', err);
        }
      }
    }
  };
  ...
  const processOrderSuccess = async (order: Order) => {
    ...
    if (idToken) {
      try {
        ...
        if (response.ok) {
          addToast('Đơn hàng đã được đồng bộ lên Cloud SQL!', 'success');
        }
      } catch (err) {
        console.error('Failed to sync order to Cloud SQL:', err);
      }
    }
  };
